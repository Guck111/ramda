...
#### Параметры:
| fn | Функция, яку необхідно обгорнути. |
:---|:---|
| вернет __function__ | Нова Функция, яка огортає функцію `fn`. Нова Функция гарантовано має арність 2. |
...
#### Параметры:
| fn | Функция, яку необхідно прив'язати до контексту. |
:---|:---|
| thisObj | Контекст, до якого прив'язується Функция `fn` |
| вернет __function__ | Функция, яка буде виконана у контексті `thisObj`. |
...
:---|:---|
| g | Інший предикат |
| вернет __function__ | Функция, що застосовує свої аргументи до `f` та `g` та `&&` їх результатів разом. |

_Добавлено в версии v0.12.0_

Функция,яка викликає дві переданих функції і вернет `&&` результату. Це поверне результат першої функції, якщо вона `false`і, а інакше поверне другу функцію. __Зауважте__, що коротке замикання, а це означає, що друга Функция не буде викликана, якщо перша поверне значення `false`.
...
#### Параметры:
| fn | Функция, яку потрвібно застосувати до лишившихся аргументів. |
:---|:---|
...
вернет результат виклику першого аргумента та залишивіся аргументи. Інколи це корисно в якості конверційнрї функції для `R.converge`: перша гілка може утворити функцію, в той час як решта гілок утворюють значення, які будуть передані у ту утворену функцію в якості аргументів.
...
#### Параметры:
| fn | Функция, з якою треба поєднати. |
:---|:---|
| list | Список для відображення. |
| вернет __Array__ | Результат плаского відображення списку `list` з функцією `fn`. |
...
| вернет __Number__ | вернет `minimum`, коли значення менше за мінімум(`value < minimum`), `maximum`, коли значення більше за максимум(`value > maximum`), в усіх інших випадках вернет саме значення(`value`) |
...
:---|:---|
| вернет __*__ | Глибоку копію переданого об'єкта/масива(`val`) |
...
:---|:---|
| вернет __function__ | Складену функцію |

_Добавлено в версии v0.1.0_

Виконує складання функції справа наліво. Крайня права Функция може мати будь-яку арність, а інші функції повинні бути унарними.
...
#### Параметры:
| after | Функция. `after` буде викликана з вернетмими значеннями `fn1` та `fn2` як її аргументи. |
:---|:---|
| functions | Список функцій. |
| вернет __function__ | Нова Функция. |

_Добавлено в версии v0.4.2_

Приймає функцію перетворювач та список функцій та вернет нову функцію. Під час виклику ця нова Функция застосовується до деяких аргументів, кожна передана Функция застосовується до відповідних аргументів. Результат кожної застосованої функції передається як аргумент до функції перетворювача для створення вернетмого значення.
